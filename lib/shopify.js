...
export async function getProducts(first = 20) {
  ...
  // ✅ Cache court pour la liste des produits (sera revalidé par webhook)
  const data = await shopifyFetch({
    query,
    variables: { first },
    cache: { next: { revalidate: 60 } } // 1 minute
  });
...
export async function getCollection(handle, first = 50) {
  ...
  // ✅ Cache court pour les collections (sera revalidé par webhook)
  const data = await shopifyFetch({
    query,
...
export async function getProduct(handle) {
  ...
  // ✅ Cache court pour les produits (permet les mises à jour rapides via webhook)
  const data = await shopifyFetch({
    query,
    variables: { handle },
    cache: { next: { revalidate: 60 } } // 1 minute (sera revalidé par webhook de toute façon)
  });
