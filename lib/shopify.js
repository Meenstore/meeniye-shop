...
// Créer un cart Shopify avec redirectUrl vers le site Next.js
export async function createCart() {
  ...
  // Maintenant on ajoute le redirectUrl via buyerIdentity
  const updateQuery = `
  ...
  if (updateData.cartBuyerIdentityUpdate.userErrors.length > 0) {
    console.error('Erreur lors de l\'ajout du redirectUrl:', updateData.cartBuyerIdentityUpdate.userErrors);
    // On continue quand même avec le cart créé
  }
...
// Récupérer une page Shopify par handle (pour contenu éditable)
export async function getPage(handle) {
  ...
  // ✅ Cache de 10 minutes pour les pages (contenu qui change rarement)
  const data = await shopifyFetch({
    query,
    variables: { handle },
    cache: { next: { revalidate: 600 } } // 10 minutes
  });
